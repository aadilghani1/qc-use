(() => {
  ...
  // Password values never leave the page: only whether the field is filled.
  ...
  for (const e of document.querySelectorAll(selector)) {
    ...
    if (secret(e)) base.password=true;
  ...
  // File inputs are usually hidden behind a styled label, so they need no geometry: nothing is clicked.
  for (const e of uploads) {
    const holder=[...(e.labels||[])].find(visible) || (visible(e) ? e : null);
    if (!holder) continue;
  ...
  const frames=[...document.querySelectorAll('iframe,frame')].filter(f=>{
    ...
    return visible(f) && r.width>20 && r.height>20 && r.bottom>0 && r.top<innerHeight;
  }).map(f=>{
    let host=''; try { host=new URL(f.src||'about:blank',location.href).host; } catch {}
    return {host,label:f.title||f.name||host||'embedded frame',same_origin:host===location.host};
  });
  const notes=frames.map(f=>`[Embedded frame: ${f.label} — its controls are not available]`);
  const text=[words.join('\n').slice(0,6000),...notes].filter(Boolean).join('\n');
